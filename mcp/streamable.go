...
func (c *streamableClientConn) Write(ctx context.Context, msg jsonrpc.Message) error {
	...
	doRequest := func() (*http.Request, *http.Response, error) {
		...
		// The caller (doWithAuth) handles wrapping the error with ErrRejected to
		// ensure the jsonrpc2 connection isn't permanently broken.
		return req, resp, err
	}
...
// doWithAuth executes an HTTP request, automatically handling OAuth2 token
// retrieval errors and 401/403 HTTP status codes by triggering the
// OAuthHandler's Authorize flow and retrying.
//
// doRequest should construct and send the HTTP request, and return the sent
// request (which may be needed for authorization), the response (if any), and
// any error.
